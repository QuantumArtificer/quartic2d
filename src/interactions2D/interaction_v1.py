...
#     C∞ bump window w(r):
#         w=1                for r <= r_start
#         w=exp(-1/(1-x^2))  for r_start < r < r_end, x=(r-r_start)/(r_end-r_start)
#         w=0                for r >= r_end
#     This is infinitely smooth at both endpoints and numerically stable.

#     Parameters
#     ----------
#     r : 1D np.array (monotone increasing)
#     r_start : float   # np.where the taper begins
#     r_end   : float   # np.where the taper reaches 0 exactly

#     Returns
#     -------
#     w : 1D np.array in [0,1]
#     """

#     if r_end <= r_start:
#         raise ValueError('r_start must be >= r_end')
    
#     w = np.ones_like(r, dtype=float)
#     w[r >= r_end] = 0.0

#     x = (r - r_start) / (r_end - r_start)
#     window = (x > 0.0) & (x < 1.0)
#     t = 1.0 - x[window]**2

#     w[window] = (np.exp(-1.0 / t))*np.e
...
class HankelTransform():
    ...
    def _interpolate(self, method = 'linear'):
        ...
        self._spline = RGI((self._r, ), self._F_r, method = method, bounds_error = False, fill_value = 0.0)
        
        def f_r(x):
            x = np.array(x)
            ndim = x.ndim
            if ndim == 2:
                f = self._spline(x.flatten())
